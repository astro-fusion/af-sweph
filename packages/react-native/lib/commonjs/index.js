...
async function createSweph(options) {
  ...
  return {
    ...
    calculateLagna(date, location, options) {
      ...
      const ascendant = (0, _swephCore.normalizeLongitude)(result.ascmc[0] - ayanamsaVal);
      const houses = result.cusp.slice(1, 13).map(c => (0, _swephCore.normalizeLongitude)(c - ayanamsaVal));
      return {
        longitude: ascendant,
        rasi: (0, _swephCore.getRashi)(ascendant),
        rasiDegree: (0, _swephCore.getRashiDegree)(ascendant),
        houses
      };
    },
