...
function gmst(jd: number): number {
    ...
    return norm360(gmstSeconds / 240); // seconds → degrees
}
 ...
 */
function ascendantTropical(lst: number, lat: number, eps: number): number {
 ...
 */
export function getFastLagna(
    date: Date,
    latitude: number,
    longitude: number,
    ayanamsaType: number = AYANAMSA_TYPE.LAHIRI
): LagnaResult {
